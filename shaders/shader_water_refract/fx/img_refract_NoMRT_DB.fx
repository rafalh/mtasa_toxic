...
{
    return ( gProjectionMainScene[3][2] / linDepth ) + gProjectionMainScene[2][2];
}
...
float4 PixelShaderFunctionSM3NoDB(PSInput PS) : COLOR0
{
    ...
    float applyFade = saturate(1 - MTAApplyFade(PS.WorldPos, PS.CameraPosition));
    ...
    // get pixel depth
    float matDepth = PS.Depth.x / PS.Depth.y;
    float expMatDepth = invLinearizeDepth(matDepth);
	
    // calculate bleed fix
    float depthAlt = Linearize(FetchDepthBufferValue(TexProj.xy + NormalTex.xy));
    float refMul =  1 - saturate(matDepth - depthAlt);
    ...
    // mask pixel if material depth > scene depth
    float depthVal = FetchDepthBufferValue(TexProj.xy);
    if (expMatDepth >= depthVal) finalColor.a = 0;
	
    return saturate(finalColor);
}

float4 PixelShaderFunctionSM2NoDB(PSInput PS) : COLOR0
{
    // slice if needed
    float2 distFromCam = float2( distance(PS.CameraPosition.x, PS.WorldPos.x), distance(PS.CameraPosition.y, PS.WorldPos.y));
    if (((distFromCam.x < sElementSize.x * 0.5) && (distFromCam.y < sElementSize.y * 0.5)) && !bIsDetailed) return 0;
    ...
    // include fog fading effect
    float applyFade = 1 - MTAApplyFade(PS.WorldPos, PS.CameraPosition);
...
// Techniques
//--------------------------------------------------------------------------------------
technique dxDrawImage4D_ref_PS3_NoMRT_DB
{
  pass P0
  {
    ZEnable = true;
...
technique dxDrawImage4D_ref_PS2_NoMRT_DB
{
  pass P0
  {
    ZEnable = true;
    ...
    PixelShader  = compile ps_2_0 PixelShaderFunctionSM2NoDB();
  }
} 
