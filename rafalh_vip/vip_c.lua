...
local g_IsVip = false
local g_NeonColorWnd, g_VehColorWnd, g_VehColor1Wnd, g_VehColor2Wnd, g_VehLightsColorWnd, g_NametagColorWnd = nil, nil, nil, nil, nil, nil
local g_VipEnd
local g_Skins = {}
...
local function VipLoadSkins ()
	local node, i = xmlLoadFile ( ":admin/conf/skins.xml" ), 0
	if ( node ) then
		local loaded = {}
		while ( true ) do
			local group_node, j = xmlFindChild ( node, "group", i ), 0
			if ( not group_node ) then break end
			i = i + 1
			
			while ( true ) do
				local skin_node = xmlFindChild ( group_node, "skin", j )
				if ( not skin_node ) then break end
				j = j + 1
				
				local skin = {}
				skin.id = tonumber ( xmlNodeGetAttribute ( skin_node, "model" ) )
				skin.name = xmlNodeGetAttribute ( skin_node, "name" )
	...
	table.sort ( g_Skins, function ( skin1, skin2 ) return skin1.name < skin2.name end )
end
