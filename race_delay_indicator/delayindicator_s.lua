...
addEventHandler('onGamemodeMapStart', g_Root,
	function(mapres)
		if g_Debug then outputDebugString("delay_indicator: sending data: "..tostring(mapInfo.name)) end
		...
		if #sql > 0 then
			--if g_Debug then outputDebugString(tostring(sql[1].playername).." "..tostring(sql[1].interims)) end
		...
		if g_Debug then outputDebugString(tostring(topTimeRankPlayer[1]).." "..tostring(topTimeRankPlayer[2])) end
	end
)
...
addEventHandler('onPlayerReachCheckpoint', g_Root,
	function(checkpointNum, timePassed)
        ...
        if g_Debug then outputDebugString("race_delay_indicator: ".."info for "..getPlayerName(source)) end
        ...
        if rank == 1 then
			if topTimeInterims and topTimeRankPlayer and topTimeInterims[checkpointNum] then
				...
				if g_Debug then outputDebugString("race_delay_indicator: "..getPlayerName(source).." "..diff.." record  #"..topTimeRankPlayer[1]) end
            ...
            if g_Debug then outputDebugString("race_delay_indicator: ".."frontPlayer = "..tostring(frontPlayer)) end
            if not frontPlayer then
                if g_Debug then outputDebugString("race_delay_indicator: ".."no frontPlayer use lastPlayer") end
                if lastPlayer[checkpointNum] then
                    frontPlayer = lastPlayer[checkpointNum]
                else
                    if g_Debug then outputDebugString("race_delay_indicator: ".."no lastPlayer use nothing") end
                end
			else
				if allCpTimes[frontPlayer] and allCpTimes[frontPlayer][checkpointNum] then
					if g_Debug then outputDebugString("race_delay_indicator: ".."use allCpTimes["..getPlayerName(frontPlayer).."]["..checkpointNum.."] = "..tostring(allCpTimes[frontPlayer][checkpointNum])) end
					diff = timePassed - allCpTimes[frontPlayer][checkpointNum]
				else
					if g_Debug then outputDebugString("race_delay_indicator: ".."lua is too slow no difference") end
					diff = 0
				end
				if not players[source] then
		...
		if g_Debug then outputDebugString("race_delay_indicator: ".."allCpTimes: "..tostring(getPlayerName(source)).." checkpointNum = "..tostring(checkpointNum).." timePassed = "..tostring(allCpTimes[source][checkpointNum])) end
	end
)
...
addEventHandler("onPlayerToptimeImprovement", g_Root,
	function(newPos)
		if g_Debug then outputDebugString(tostring(getPlayerName(source)).." "..tostring(newPos).." "..tostring(topTimeRankPlayer[1])) end
		if newPos <= topTimeRankPlayer[1] and allCpTimes[source] then
			...
			if #sql > 0 then
				if g_Debug then outputDebugString("mapinterims: update mapinterims "..playername.." "..interims) end
				executeSQLQuery("UPDATE mapinterims SET playername=?, interims=? WHERE mapname=?", playername, interims, mapName )
			else
				if g_Debug then outputDebugString("mapinterims: insert mapinterims "..playername.." "..interims) end
...
function updatetopTimeInterims()
	...
	if g_Debug then outputDebugString("mapinterims: start update "..tostring(#sql)) end
