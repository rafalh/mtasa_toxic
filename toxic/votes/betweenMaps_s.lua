...
local function onPollStarting(poll)
    ...
    if (mapTypeVote) then
        ...
        for i, mapType in ipairs(g_MapTypes) do
            local forced =
                g_PlayersCount > 1 and mapType.max_others_in_row and mapType.others_in_row >= mapType.max_others_in_row
            if (forced) then
        ...
        if (not forcedExist) then
            ...
            for i, mapType in ipairs(g_MapTypes) do
                local forced =
                    g_PlayersCount > 1 and mapType.max_others_in_row and
                    mapType.others_in_row >= mapType.max_others_in_row
                if (not forced) then
