...
onClientRenderHandler = function ()
	...
	local value = math.sqrt ( vx^2 + vy^2 + vz^2 )/1.5
	if ( value > 1 ) then
		value = 1
	end
...
g_WidgetCtrl[$(wg_show)] = function ( b )
	...
	g_Show = b
	if ( b ) then
		addEventHandler ( "onClientRender", g_Root, onClientRenderHandler )
	else
		removeEventHandler ( "onClientRender", g_Root, onClientRenderHandler )
	end
end
...
g_WidgetCtrl[$(wg_reset)] = function ()
	...
	g_Pos = { g_ScreenSize[1] - g_Size[1] - 24, g_ScreenSize[2]*0.98 - g_Size[2] - 120 }
	recalc ()
...
#VERIFY_SERVER_BEGIN ( "63F48BA32C6EED46CEABEB38FA458739" )
	g_WidgetCtrl[$(wg_reset)] () -- reset pos, size, visiblity
	triggerEvent ( "onRafalhAddWidget", g_Root, getThisResource (), "Speed bar" )
