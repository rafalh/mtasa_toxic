...
onClientRender = function ()
	...
	for i = 1, buf:len (), 1 do
		local c = buf:sub ( i, i )
		if ( c ~= ":" ) then
			drawDigit ( tonumber ( c ), x + w/6, g_Pos[2] + g_Size[2]/10, w, g_Size[2]*8/10, tocolor ( 0, 255, 0 ) )
			x = x +	w*8/6
		else
			x = x + w/6
...
addEventHandler ( "onEvent_rafalh", g_Root, function ( event, code )
	if ( event == 10 ) then -- onVerifyServerReply, sourceResource==getThisResource() mta bug?
		if ( code == verification_code ( g_Me, getResourceName ( getThisResource () ) ) ) then
...
--if ( md5 ( getResourceName ( getThisResource () ) ) == "" ) then
