...
function addPlayerTime ( player_id, map_id, time )
	...
	if ( pos <= 3 and not top ) then -- player joined to the top
		...
		if ( rows and rows[1] ) then -- someone left the top
			...
			DbQuery ( "UPDATE rafalh_besttimes SET rec='', cp_times='' WHERE player=? AND map=?", pl4_id, map_id )
		end
	end
...
function BtSendMapInfo (room, show, player)
	...
	local rating = ( rows[1].rates_count > 0 and rows[1].rates/rows[1].rates_count ) or 0
	local author = map:getInfo("author")
	triggerClientInternalEvent ( player or g_Root, $(EV_CLIENT_MAP_INFO), g_Root, show, map_name, author, rows[1].played, rating, rows[1].rates_count, g_TopTimes )
end
...
function getTopTime ( map_res, cp_times )
	...
	local rows
	if ( cp_times ) then
		rows = DbQuery ( "SELECT cp_times, time FROM rafalh_besttimes WHERE map=? AND cp_times<>'' ORDER BY time LIMIT 1", map_id )
	end
